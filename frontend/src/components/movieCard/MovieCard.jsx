        ...
        >
            <div className='relative  w-full   rounded-[12px]  aspect-[1/1.5] bg-cover bg-center mb-[30px] flex flex-col items-end justify-between ' style={{"boxShadow":"0 4px 8px rgba(0,0,0,0.1)"}}>
                <div className='  w-full h-full rounded-[12px] overflow-hidden'>
                        <Img classname="w-full h-full   object-cover object-center" src={data?.img_link} />
                </div>
                {!fromSearch && (
                    <React.Fragment>
                        ...
                        <div className='mt-2 text-[12px] md:text-[16px] opacity-[0.5] mb-2'>
                            <Genres data={data?.genres?.split("|").slice(0,2)} />
                        </div>
                    </React.Fragment>
                )}
            </div>
            <div className="text-white flex flex-col ">
            <span className='text-[12px]  md:text-[18px] whitespace-nowrap text-ellipsis overflow-hidden max-w-full'>{data.title.slice(0,-6)}</span>
            <span className='mt-2 text-[12px] md:text-[16px] opacity-[0.5] mb-2'>{dayjs(data.title.slice(-5,-1)).format("YYYY")}</span>
            </div>
        </div>
